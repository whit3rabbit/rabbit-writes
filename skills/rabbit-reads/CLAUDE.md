...
- **The safety band is unsuppressible in `check_notes.py`.** Every doc and every freeform file goes through `scan_problems(..., safety_only=not args.scan)`, so the concealed-directive findings run even on a bare check, the way they already do unsuppressed in `rwlib/suppress.py` and `--apply-safe`. What `--scan` buys is everything else in the battery plus voice-profile forwarding; what no flag combination buys is a way to skip the safety scan, because an unchecked note set is exactly where a planted directive would want to live.
