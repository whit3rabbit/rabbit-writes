...
python3 skills/rabbit-reads/scripts/check_notes.py <notes-dir> --book-type <type> --source scratch/<normalized-source>.txt
...
- **`SKILL.md`**: Frontmatter, three modes (`distill`, `extend`, `verify`), and the seven-phase workflow: normalize, map, plan, fan out, index, verify, deliver.
  ...
  - `map_structure.py`: Maps section headings to line ranges over the normalized text (ATX and setext markdown headings included) and cuts them into fan-out batches with per-batch word and token estimates. `--book-type` selects the segmentation grammar from `references/book-types/`.
  - `check_notes.py`: Verifies a notes folder against its book type: line band, template sections, kind markers, Source line shape, README index, See also links. The engine safety band (concealed directives, injection) runs on every check and cannot be turned off. `--scan` adds the full scanner battery, and `--voice-rules` passes a profile through, with or without `--scan`. `--source` checks the paraphrase rule against the normalized source.
- **`references/book-types/`**: One file per source type, the one home of that type's contract. The header lines (`**Kind markers:**`, `**Length band:**`, `**Template sections:**`, `**Source line:**`, `**Free-form files:**`) are parsed by `check_notes.py` and by the repo `scripts/validate.py`, so a new type is a new file following the grammar and not a code change.
- **`references/layouts/`**: One file per folder shape (flat `cheatsheets`, Obsidian-style `obsidian`), the one home of that layout's contract. Its six header lines (`**Index file:**` through `**Folders:**`) are parsed by `check_notes.py --layout` and by `scripts/validate.py`.
...
- **The safety band is unsuppressible in `check_notes.py`.** Every doc and every freeform file goes through `scan_problems(..., safety_only=not args.scan)`, so the concealed-directive findings run even on a bare check, the way they already do unsuppressed in `rwlib/suppress.py` and `--apply-safe`. What `--scan` buys is everything else in the battery plus voice-profile forwarding. No flag combination buys a way to skip the safety scan, because an unchecked note set is exactly where a planted directive would want to live.
- **`scan_problems` reads structured findings, never the report.** It runs `--check --json` and pins `findings.SCHEMA_VERSION`, because a heading-based grep over the human report is the fragile consumer `rwlib/findings.py`'s docstring warns against: a cosmetic change to that heading breaks the summary silently while the failure still says "P0 finding present".
